...
#### [P4-C1] Lateral movement: JWT de cliente/profissional aceito em endpoints de admin (não-`requireAdmin`)
...
#### [P4-C2] `POST /api/bloqueios` aceita `salaoId` do body + `DELETE /api/bloqueios/:id` sem filtro de tenant
...
#### [P4-C3] `POST /api/fidelidade/adicionar` — pontos arbitrários sem tenancy de cliente
...
#### [P4-C4] `POST /api/fidelidade/resgatar` — race condition permite double-spend de pontos
...
#### [P4-A1] `VendaService.cancelar` restaura estoque sem filtro de tenancy
...
#### [P4-A2] `AtendimentoService.criar` — sem tenancy de FKs + `valor` arbitrário
...
#### [P4-A3] `PUT /api/atendimentos/:id` permite alterar `valor` arbitrariamente
...
#### [P4-A4] `/api/configuracoes PUT /` — mass-assignment de chave/valor arbitrários
...
#### [P4-A5] `GET /api/configuracoes/navegadores` — information disclosure do servidor (fs paths)
...
#### [P4-A6] `POST /api/auth/register` — endpoint público permite criar salão SEM rate limit específico
...
#### [P4-A7] Endpoint `/api/ai/command` executa `create_agendamento` sem validação do `clienteId` em `req.salaoId`
...
#### [P4-M1] `jwt.verify` chamado sem `algorithms` explicit — confia no default da lib
...
#### [P4-M2] `emailService.sendPasswordResetEmail` hardcoded `http://localhost:3000/reset-password/${token}`
...
#### [P4-M3] `POST /api/notificacoes` permite criar notificação sem validar `cliente_id` cross-tenant
...
#### [P4-M4] WebSocket: `JSON.parse(message)` sem proteção contra mensagens >1MB
...
#### [P4-M5] `npm production` audit limpo, mas Node 26 em CI/dev local — incompatível com Render
...
#### [P4-M6] `/api/sync/push` aceita até 500 mudanças sem validação extra de payload total
...
#### [P4-M7] `bcrypt.compare` precedido de branch que pode vazar existência de usuário (timing)
...
#### [P4-M8] `req.query` arrays via parameter pollution não testado
...
#### [P4-B1] `JWT_SECRET` mesmo para JWT admin e cliente/profissional
...
#### [P4-B2] `bcrypt.compare` retorna em tempo variável conforme tamanho do hash
...
#### [P4-B3] `/api/health` retorna `services: { database: 'connected' }` sem validar pool ativo
...
#### [P4-B4] Cookies não usados, mas `credentials: true` em CORS habilita possíveis side-effects futuros
...
#### [P4-B5] WebSocket `verifyClient` não valida `decoded.type` — token cliente abre WS em /ws sem restrição
...
#### [P4-B6] `console.log` de comando IA loga primeiros 200 chars de `command` (PII potencial)
...
#### [P4-B7] `GET /api/configuracoes/navegadores` — duplicado em B5 acima como A5 (mais grave por info disclosure)

#### [P4-B8] `BookupService.ALLOWED_COLUMNS` permite `senha_hash` em profissionais durante restore
...
#### [P4-B9] `securityService.js` (não auditado) — verificar se há mais utilities expostas
- **Arquivo:** `src/services/securityService.js`
- **Descrição:** Não auditado em detalhe nesta passada. Recomendado leitura na próxima.

#### [P4-B10] `crossOriginResourcePolicy` não setado em helmet — default `same-origin` em helmet@7
