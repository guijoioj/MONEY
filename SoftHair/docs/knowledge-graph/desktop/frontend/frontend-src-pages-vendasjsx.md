...
**Repository:** Desktop
**File:** `frontend/src/pages/Vendas.jsx`
**Language:** `jsx`
...
Arquivo `frontend/src/pages/Vendas.jsx` do repositório Desktop.
    ...
    <div>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        <div className="flex flex-col sm:flex-row gap-4 items-end">
          <div>
            <label className="block text-sm text-gray-500 mb-1">Data específica</label>
          ...
          <div>
            <label className="block text-sm text-gray-500 mb-1">Até</label>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {vendas?.data?.data?.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-6 py-8 text-center text-gray-500">Nenhuma venda encontrada</td>
                </tr>
              ) : (
                vendas?.data?.data?.map((venda) => (
                  <tr key={venda.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-gray-800">{formatDate(venda.data)}</td>
                    <td className="px-6 py-4 text-gray-600">{venda.clienteNome || 'Consumidor Final'}</td>
                    <td className="px-6 py-4 text-gray-600">{venda.vendedorNome || '-'}</td>
                    <td className="px-6 py-4 text-gray-600">{FORMA_PAGAMENTO_LABEL[venda.formaPagamento] || '-'}</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                <div className="col-span-2">
                  ...
                  <input type="text" value={formData.observacoes} onChange={(e) => setFormData({ ...formData, observacoes: e.target.value })}
                    placeholder="Opcional" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" />
                </div>
              </div>

              <div className="mb-6">
                <h3 className="font-medium text-gray-700 mb-3">Adicionar Itens</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm text-gray-500 mb-2">Produtos</label>
                  ...
                  <div>
                    <label className="block text-sm text-gray-500 mb-2">Serviços</label>
                    <div className="space-y-2 max-h-40 overflow-y-auto">
              ...
              <div className="mb-6">
                <h3 className="font-medium text-gray-700 mb-3">Itens da Venda</h3>
                {itens.length === 0 ? (
                    ...
                    {itens.map((item, index) => (
                      <div key={index} className="flex items-center justify-between p-3">
                        <div className="flex-1">
                          <div className="font-medium text-gray-800">{item.itemNome}</div>
                          <div className="text-sm text-gray-500">{formatCurrency(item.precoUnitario)} cada</div>
                        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b">
              ...
              <button onClick={() => setViewVenda(null)} className="text-gray-400 hover:text-gray-600"><X size={24} /></button>
            </div>
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div><p className="text-sm text-gray-500">Data</p><p className="font-medium">{formatDate(viewVenda.data)}</p></div>
                <div><p className="text-sm text-gray-500">Cliente</p><p className="font-medium">{viewVenda.clienteNome || 'Consumidor Final'}</p></div>
                <div><p className="text-sm text-gray-500">Vendedor</p><p className="font-medium">{viewVenda.vendedorNome || '—'}</p></div>
                <div><p className="text-sm text-gray-500">Forma de Pagamento</p><p className="font-medium">{FORMA_PAGAMENTO_LABEL[viewVenda.formaPagamento] || '—'}</p></div>
                <div><p className="text-sm text-gray-500">Total</p><p className="font-bold text-xl text-indigo-600">{formatCurrency(viewVenda.total)}</p></div>
              </div>
              ...
              {viewVenda.observacoes && (
                <div className="mt-4"><p className="text-sm text-gray-500">Observações</p><p className="text-gray-700">{viewVenda.observacoes}</p></div>
              )}
            </div>
          </div>
        </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl w-full max-w-md p-6">
            <h2 className="text-lg font-bold text-gray-800 mb-2">Excluir Venda</h2>
