...
export default function Notificacoes() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 bg-white dark:bg-gray-800 rounded-lg p-2 shadow">
        {[{ value: 'todas', label: 'Todas' }, { value: 'nao-lidas', label: 'Não lidas' }].map((opt) => (
          <button
            key={opt.value}
            onClick={() => setFiltro(opt.value)}
            className={`flex-1 px-4 py-2 rounded-lg transition-colors ${
              filtro === opt.value ? 'bg-indigo-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 dark:bg-gray-700'
            }`}
          >
            {opt.label}
          </button>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {notificacoes.map((notif) => (
            <div
              key={notif.id}
              className={`bg-white dark:bg-gray-800 rounded-xl shadow p-4 transition-all ${notif.lida ? 'opacity-60' : 'border-l-4 border-indigo-500'}`}
            >
              <div className="flex items-start gap-4">
                <div className="p-2 bg-gray-100 dark:bg-gray-700 rounded-lg">{getIcon(notif.tipo)}</div>
