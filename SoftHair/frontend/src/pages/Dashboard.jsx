...
export default function Dashboard() {
  ...
  const loadDashboardData = useCallback(async (showSpinner = true) => {
    try {
      ...
      const [clientesRes, servicosRes, produtosRes, agendamentosRes, atendimentosRes, profissionaisRes, aniversariantesRes] = await Promise.all([
        ...
        api.get('/clientes/aniversariantes').catch(() => ({ data: { data: [] } }))
      ]);
      ...
      setAniversariantes(Array.isArray(aniversariantesRes.data?.data) ? aniversariantesRes.data.data : []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">Aniversariantes da Semana</h2>
        {aniversariantes.length > 0 ? (
          <div className="divide-y">
            {aniversariantes.map((cliente) => {
              const telefone = (cliente.telefone || '').replace(/\D/g, '');
              const waLink = `https://wa.me/55${telefone}?text=Feliz%20Anivers%C3%A1rio%20${encodeURIComponent(cliente.nome)}!`;
              const dia = cliente.dia || new Date(cliente.dataNascimento).getDate();
              const mes = cliente.mes || (new Date(cliente.dataNascimento).getMonth() + 1);
              return (
                <div key={cliente.id} className="flex items-center justify-between py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Dia {dia}/{mes}</p>
                    </div>
                  </div>
                  {telefone && (
                    <a
          ...
          <p className="text-gray-500 text-sm">Nenhum aniversariante esta semana</p>
        )}
      </div>
