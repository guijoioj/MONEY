  ...
  static async registerDevice(userId, deviceInfo) {
    try {
      ...
      const existing = await pool.query(
        'SELECT * FROM dispositivos WHERE usuario_id = $1 AND info->>\'fingerprint\' = $2',
      ...
      if (existing.rows.length > 0) {
        ...
        await pool.query(
          ...
          [deviceInfo.appVersion, existing.rows[0].id]
        );
      ...
      await pool.query(
        `INSERT INTO dispositivos (id, usuario_id, info, ativo, criado_em, app_version)
         VALUES ($1, $2, $3, true, NOW(), $4)`,
        [deviceId, userId, JSON.stringify(deviceInfo), deviceInfo.appVersion]
      );
      return deviceId;
    } catch (error) {
