...
// Lógica reutilizável (sem HTTP) — chama dentro da MESMA transação do client passado.
// Retorna { ok, code, body } pra POST /converter/:id e POST /converter-todos compartilharem.
async function _converterUmAgendamento(client, id, salaoId) {
  ...
  if (!ag.rows.length) return { ok: false, code: 404, body: { success: false, error: 'Agendamento não encontrado', agendamento_id: id } };
  const a = ag.rows[0];
  if (a.atendimento_id) {
    return { ok: true, code: 200, body: { success: true, data: { agendamento_id: id, atendimento_id: a.atendimento_id, ja_convertido: true } } };
  }
  ...
  return { ok: true, code: 201, body: { success: true, data: { agendamento_id: id, atendimento_id: atendimentoId } } };
}

// POST /converter/:id — wrapper transacional.
router.post('/converter/:id', authMiddleware, writeGuard, async (req, res) => {
  try {
    const { withTransaction } = require('../config/database');
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ success: false, error: 'ID inválido' });
    const result = await withTransaction((client) => _converterUmAgendamento(client, id, req.salaoId));
...
// POST /converter-todos — chama a função interna em loop, sem HTTP loopback.
// Cada item roda em sua própria transação (isolamento por item; um falhar não derruba os outros).
router.post('/converter-todos', authMiddleware, writeGuard, async (req, res) => {
  try {
    ...
    const detalhes = [];
    let convertidos = 0, ja_convertidos = 0, falhas = 0;
    for (const r of rows) {
      try {
        const ret = await withTransaction((client) => _converterUmAgendamento(client, r.id, req.salaoId));
        if (ret.ok) {
          if (ret.body?.data?.ja_convertido) ja_convertidos++;
          else convertidos++;
        } else {
          falhas++;
        }
        detalhes.push({ id: r.id, code: ret.code, ok: ret.ok });
      } catch (e) {
        falhas++;
        detalhes.push({ id: r.id, code: 500, ok: false, error: e.message });
      }
    }
    res.json({ success: true, data: { total: rows.length, convertidos, ja_convertidos, falhas, detalhes } });
  } catch (error) {
